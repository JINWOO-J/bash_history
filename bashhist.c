     ...
     const char *line;
{
  ...
  if (strlen(line) < SYSLOG_MAXLEN)
    ...
    syslog (SYSLOG_FACILITY|SYSLOG_LEVEL, "[ LC_IP:%s IP:%s LUID=%s CUID=%s TTY:%s PWD:%s ] %s",strtok(getenv("LC_IP")," "),  strtok(getenv("SSH_CLIENT")," "), getlogin(), getenv("USER"), ttyname(0), cwd, line);
    }
  else
    {
